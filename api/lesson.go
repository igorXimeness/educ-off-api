...
func (api LessonAPI) Register(e *echo.Echo) {
	...
	v1.POST("/subjects/:subject_name/modules/:module_name/lessons", api.CreateLesson)
}
...
func (api SubjectAPI) CreateLesson(c echo.Context) error {
	...
	err := api.subjectService.CreateLesson(c.Request().Context(), lesson)
    if err != nil {
        return c.JSON(http.StatusInternalServerError, map[string]interface{}{"error": err.Error()})
    }
